...
import { AuthService } from '@core/services/auth.service'; // ✅ 1. Import nécessaire
...
export class MyStudentsComponent implements OnInit {
    ...
    constructor(
        ...
        private authService: AuthService // ✅ 2. Injection du service d'auth
    ) {}
    ...
    loadData() {
        // ✅ 3. Récupérer l'ID du directeur connecté
        const currentDirector = this.authService.currentUser();

        if (!currentDirector || !currentDirector.id) return;

        this.userService.getUsersByRole('DOCTORANT').subscribe({
            next: (data) => {
                // ✅ 4. Filtrer : Garder uniquement ceux dont le directeurId correspond à mon ID
                const myStudentsOnly = data.filter(u => u.directeurId === currentDirector.id);

                // Simulation de données (si le backend ne les renvoie pas encore)
                const enrichedData = myStudentsOnly.map(u => ({
                    ...u,
                    anneeThese: u.anneeThese || Math.floor(Math.random() * 6) + 1,
