        ...
        <app-main-layout>
            <div class="page-container p-4">
                ...
                <div class="mb-4 d-flex gap-2 flex-wrap">
                    ...
                    <button class="btn btn-sm"
                            [class.btn-danger]="selectedStatut() === 'REJETE'"
                            [class.btn-outline-danger]="selectedStatut() !== 'REJETE'"
                                        ...
                                        <td class="ps-4">
                                            <div class="d-flex align-items-center gap-3">
                                                <div class="avatar-circle shadow-sm">
                                                    {{ getDoctorantInitial(ins) }}
                                                </div>
                                                <div>
                                                    <div class="fw-bold text-dark">{{ getDoctorantNom(ins) }}</div>
                                                            ...
                                                            <div class="detail-card h-100">
                                                                ...
                                                                <div class="mt-3">
                                                                    ...
                                                                    <div class="mb-3">
                                                                        ...
                                                                        <div>{{ ins.laboratoireAccueil || 'Non spécifié' }}</div>
                                                                    </div>
...
export class AdminInscriptionValidationComponent implements OnInit {
    ...
    /**
     * Récupère le nom complet du doctorant depuis l'objet enrichi ou l'ID
     */
    getDoctorantNom(ins: Inscription): string {
        // Si l'objet doctorant est joint (enrichi côté backend ou frontend)
        if (ins.doctorant) {
        ...
        return `Doctorant #${ins.doctorantId}`;
    }

    /**
     * Récupère l'initiale pour l'avatar
     */
    getDoctorantInitial(ins: Inscription): string {
        if (ins.doctorant) {
            const prenom = ins.doctorant.prenom || ins.doctorant.firstName || '';
            const nom = ins.doctorant.nom || ins.doctorant.lastName || '';
    ...
    confirmValidate(ins: Inscription, event: Event) {
        ...
        if (confirm(`Valider l'inscription de ${this.getDoctorantNom(ins)} ?`)) {
    ...
    confirmRefusal(ins: Inscription, event: Event) {
        ...
        if (confirm(`Rejeter l'inscription de ${this.getDoctorantNom(ins)} ?`)) {
